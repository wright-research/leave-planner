...
export async function setAnchor(patch) {
  ...
  if (!DEMO_MODE) {
    await supabase.from('balances').upsert({ id: 1, ...state.balances });
  }
  notify();
}
...
export async function upsertCalendarEntry(date, patch) {
  ...
  if (!DEMO_MODE) {
    await supabase.from('calendar_entries').upsert(merged);
  }
  notify();
}
...
export async function deleteCalendarEntry(date) {
  ...
  if (!DEMO_MODE) {
    await supabase.from('calendar_entries').delete().eq('date', date);
  }
  notify();
}
...
export async function logReload({ reload_date, booklets_added, cash_added, fsa_spent, notes }) {
  ...
  if (!DEMO_MODE) {
    await supabase.from('reload_log').insert(entry);
  }
  notify();
}
