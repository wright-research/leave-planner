...
import {
  ...
  FSA, FSA_PAYCHECKS_PER_MONTH_CAP, PAYCHECK_ANCHOR, LEAVE,
} from '../constants.js';
...
export function leaveAccrualForDate(date) {
  if (!isPaycheckFriday(date)) return { annual: 0, sick: 0 };
...
export function projectCardBalances({
  ...
  for (let i = 1; i <= days; i++) {
    ...
    if (isCommuteDay(date, entry?.commute_override)) {
      const trips = TRIPS_PER_COMMUTE_DAY;
...
export function projectLeaveBalance({
  ...
  if (days > 0) {
    for (let i = 1; i <= days; i++) {
      const date = addDays(anchorDate, i);
      if (isPaycheckFriday(date)) balance += accrualPerPaycheck;
